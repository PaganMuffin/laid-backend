(() => {
  ...
  function h2(u) {
    ...
    return e.body = "", e.finished = false, e.status = e.statusCode = 200, e.getHeaders = () => Object.fromEntries(r), e.getHeaderNames = () => [...r.keys()], e.hasHeader = r.has.bind(r), e.getHeader = r.get.bind(r), e.removeHeader = r.delete.bind(r), e.setHeader = r.set.bind(r), Object.defineProperty(e, "status", {
      ...
      let f2 = o2[i2] || e.getHeader(i2), l3 = o2[s] || e.getHeader(s);
      t == null ? t = "" : a2 === "object" ? (t = JSON.stringify(t), l3 = l3 || "application/json;charset=utf-8") : a2 !== "string" && (t = String(t)), o2[s] = l3 || "text/plain", o2[i2] = f2 || String(t.byteLength || h(t)), n2 === 204 || n2 === 205 || n2 === 304 ? (e.removeHeader(i2), e.removeHeader(s), delete o2[i2], delete o2[s], t = null) : u === "HEAD" && (t = null), e.writeHead(n2, o2), e.end(t);
    }, e;
  }
  ...
  function x() {
    ...
    return t = {
      ...
      onerror(s2, r, e, n2) {
        let a2 = c2[e = e || 500], i3 = n2 && n2.message || a2 || String(e);
  ...
  function setCookies(req, res, cookies) {
    const strCookies = cookies.map((c3) => serializeCookie(c3, isSecureEnvironment(req)));
  ...
  function uuid() {
    return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(c3) {
      var r = Math.random() * 16 | 0, v = c3 == "x" ? r : r & 3 | 8;
  ...
  var PostgrestTransformBuilder = class extends PostgrestBuilder {
    select(columns = "*") {
      let quoted = false;
      const cleanedColumns = columns.split("").map((c3) => {
        if (/\s/.test(c3) && !quoted) {
          return "";
        }
        if (c3 === '"') {
  ...
  var convertColumn = (columnName, columns, records, skipTypes) => {
    let column = columns.find((x2) => x2.name == columnName);
  ...
  function l2(o2, t, s2) {
    let e = t ? { ...a, ...t } : a;
    o2.setHeader("Access-Control-Allow-Origin", e.origin), e.origin !== "*" && o2.headers.append("Vary", "Origin"), e.credentials && o2.setHeader("Access-Control-Allow-Credentials", "true"), e.expose.length && o2.setHeader("Access-Control-Expose-Headers", e.expose), s2 && (e.maxage != null && o2.setHeader("Access-Control-Max-Age", e.maxage), e.methods.length && o2.setHeader("Access-Control-Allow-Methods", e.methods), e.headers.length && o2.setHeader("Access-Control-Allow-Headers", e.headers));
  }
  function g(o2 = {}) {
    let t = o2.origin = o2.origin || "*", s2 = typeof t == "string";
    return function(e, i3) {
      let r, n2 = e.method === "OPTIONS";
      s2 || (r = e.headers.get("Origin") || "", o2.origin = t === true && r || t instanceof RegExp && t.test(r) && r || "false"), l2(i3, o2, n2), n2 && (o2.headers || (r = e.headers.get("Access-Control-Request-Headers"), r && i3.setHeader("Access-Control-Allow-Headers", r), i3.headers.append("Vary", "Access-Control-Request-Headers")), i3.statusCode = 204, i3.end(null));
    };
  }

  // src/cda/decode.js
  var decode = (a2) => {
    String.fromCharCode(("Z" >= a2 ? 11 : 344) >= (c = a2.charCodeAt(0) + 22) ? c : c - 11);
    a2 = a2.replace("_XDDD", "");
    a2 = a2.replace("_CDA", "");
    a2 = a2.replace("_ADC", "");
    a2 = a2.replace("_CXD", "");
    a2 = a2.replace("_QWE", "");
    a2 = a2.replace("_Q5", "");
    a2 = a2.replace("_IKSDE", "");
    a2 = K(a2);
    a2 = ba(a2);
    return a2;
  };
  var K = (a2) => {
    return a2.replace(/[a-zA-Z]/g, function(a3) {
      return String.fromCharCode(("Z" >= a3 ? 90 : 122) >= (a3 = a3.charCodeAt(0) + 13) ? a3 : a3 - 26);
    });
  };
  var ba = (a2) => {
    a2 = K(a2);
  ...
  var aa = (a2) => {
    String.fromCharCode(("Z" >= a2 ? 82 : 132) >= (c = a2.charCodeAt(0) + 11) ? c : c - 55);
    return L(a2);
  };
  var L = (a2) => {
    for (var b = [], e = 0; e < a2.length; e++) {
      var f2 = a2.charCodeAt(e);
      b[e] = 33 <= f2 && 126 >= f2 ? String.fromCharCode(33 + (f2 + 14) % 94) : String.fromCharCode(f2);
    }
    return da(b.join(""));
  };
  var ca = (a2) => {
  ...
  var da = (a2) => {
    a2 = a2.replace(".cda.mp4", "");
    a2 = a2.replace(".2cda.pl", ".cda.pl");
    a2 = a2.replace(".3cda.pl", ".cda.pl");
    return -1 < a2.indexOf("/upstream") ? (a2 = a2.replace("/upstream", ".mp4/upstream"), "https://" + a2) : "https://" + a2 + ".mp4";
  };
  ...
  var check_resolutions = async (cda_id) => {
    ...
    const f2 = await fetch(url, options);
    const new_url = f2.url;
    const info = new get_video_info();
    const arr = new get_video_qualities();
    const is_premium = new check_premiun();
    await new HTMLRewriter().on(`.quality-btn`, arr).on(`#mediaplayer${cda_id}`, info).on(`.xs-txt`, is_premium).transform(f2).text();
    info["premium"] = is_premium["premium"];
    info["url"] = new_url;
    return { info, arr: arr["arr"] };
  };
  var check_status = async (cda_id) => {
    const url = `https://ebd.cda.pl/620x395/${cda_id}`;
    const f2 = await fetch(url, options);
    const t = await f2.text();
    if (f2.status !== 200 || t.includes("Trwa konwersja")) {
      return {
        "exist": false,
        "partner": false,
        "premium": false
      };
    } else if (f2.url.includes("vfilm") && t.includes("Ten film jest dost\u0119pny dla u\u017Cytkownik\xF3w premium")) {
      return {
        "exist": true,
        "partner": false,
        "premium": true
      };
    } else if (f2.url.includes("vfilm")) {
      return {
        "exist": true,
        "partner": true,
        "premium": false
  ...
  var get_url = async (url, res, cda_id) => {
    const s2 = res ? `?wersja=${res}` : "";
    const f2 = await fetch(`${url}${s2}`, options);
    const video_url = new get_video_url();
    await new HTMLRewriter().on(`#mediaplayer${cda_id}`, video_url).transform(f2).text();
  ...
  var get_data = async (cda_id) => {
    ...
    const d2 = await check_resolutions(cda_id);
    const arr = d2["arr"];
    let info = d2["info"];
    if (info["premium"]) {
    ...
    if (info["title"] && info["duration"] && arr.length === 0) {
      ...
      q = await Promise.all(arr.map(async (x2) => {
        return {
          "quality": x2["quality"],
          "resolution": x2["resolution"],
          "url": await get_url(info["url"], x2["resolution"], cda_id)
        };
      }));
    }
    delete info["url"];
    ...
    <html>
        ...
        <script>
            ...
            player.source = {
                ...
                sources: ${JSON.stringify(data["data"]["qualities"].map((x2) => {
      return { "src": x2["url"], "type": "video/mp4", "size": x2["resolution"].replace("p", "") };
    }))},
...
`;
  ...
  // src/index.js
  var db = createClient("https://ygggayldbzezqqsunqdd.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlhdCI6MTYyODQyMzg3NCwiZXhwIjoxOTQzOTk5ODc0fQ.8dFMJxk51smLydcuXwtmrwYztTHuRDqe6VKpzH-yY3g");
  ...
  API.add("GET", `/${API_VERSION_V1}/add/:id`, async (req, res) => {
    ...
    const { data, error } = await db.from("urls").insert(obj);
    if (error)
      throw error;
  ...
  API.add("GET", `/${API_VERSION_V1}/json/:id`, async (req, res) => {
    ...
    const { data, error } = await db.from("urls").select("cda_id").eq("short", short).single();
    if (error && !data) {
      res.send(404, { "code": 404, "msg": "Not Found", "data": null });
      res.end();
    }
    let d2 = await get_data(data["cda_id"]);
    res.send(d2["code"], d2);
  });
  API.add("GET", `/${API_VERSION_V1}/player/:id`, async (req, res) => {
    ...
    if (d2["code"] === 200) {
      ...
      html = build_player(d2, req["url"]);
    }
    res.send(d2["code"], html ? html : d2["data"]);
  });
